...
// export const API_ENDPOINT = 'http://112.213.87.219:8080'
export const API_ENDPOINT = 'https://api.eardev.xyz/abs/api/v3'
export const API_ENDPOINT_MEDIA = 'https://api.eardev.xyz/rms/api/v3'
const token =
...
class AxiosServer {
  ...
  getConfigMedia(IsAuth, customHeader) {
    const headers = {
      'Content-Type': 'application/json',
    }
    if (token) {
  ...
  postMedia(url, body, headerCustom) {
    return this.instance.post(url, body, this.getConfigMedia(true, headerCustom))
  }

  deleteMedia(url) {
    return this.instance.delete(url, this.getConfigMedia(true))
  }
